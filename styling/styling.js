...
export const getStyleAttrs = stylesCreator => {
  return Object.keys(typeof stylesCreator === 'function' ? stylesCreator(baseTheme) : stylesCreator);
};
export const mergeStyleCreators = (...creators) => theme => {
  ...
  if (process.env !== 'production') {
    ...
    if (repeatedList.length) {
      ...
      `mui-styling: Styles collision alert! attributes: [${repeatedList.join(', ' // eslint-disable-next-line max-len
      )}]. This might cause bugs in styling, please check these style attributes`);
    }
  }

  return creators.reduce((a, b) => _extends({}, a, {}, typeof b === 'function' ? b(theme) : b), {});
};
export const attachStylingParams = (Component, stylesCreator, options = {}) => {
  ...
  const getOverrides = (css, props = {}) => {
    const {
      [camelCase(`${name}Overrides`)]: overrides
    } = props;
    return {
